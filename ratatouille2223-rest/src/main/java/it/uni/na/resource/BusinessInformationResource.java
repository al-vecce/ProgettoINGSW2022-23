...
public class BusinessInformationResource {
    ...
    public Response getBusinessInformation() {
        ...
        if(result == null) {
            return Response.ok("Non esiste alcuna Attività.").build();
        } else if (result.isEmpty() || result.isBlank()) {
            return Response.ok("L'Attività non dispone di alcuna informazione.").build();
        }
    ...
    public Response postUpdateBusinessInformation(String json_request) {
        ...
        try {
            ...
            throw new WebApplicationException("JSON Mapping Error for BUSINESS/UPDATE encountered.", 500);
        }
        catch (JsonProcessingException ex2){
            throw new WebApplicationException("JSON Parsing Error for BUSINESS/UPDATE encountered.", 500);
        }
    }
